...
"""
OnlineFinder’s locale data
=====================

The variables :py:obj:`RTL_LOCALES` and :py:obj:`LOCALE_NAMES` are loaded from
:origin:`olf/data/locales.json` / see :py:obj:`locales_initialize` and
...
.. hint::
   ...
   :origin:`olf/data/locales.json` needs to be rebuild::

     ./manage data.locales

OnlineFinder's locale codes
======================

.. automodule:: olf.sxng_locales
   :members:


OnlineFinder’s locale implementations
================================
"""
...
"""Additional languages OnlineFinder has translations for but not supported by
...
def get_translation_locales() -> list[str]:
    ...
    generated from the translation folders in :origin:`olf/translations`"""
...
def locales_initialize():
    """Initialize locales environment of the OnlineFinder session.
...
def region_tag(locale: babel.Locale) -> str:
    """Returns OnlineFinder's region tag from the locale (e.g. zh-TW , en-US)."""
...
def language_tag(locale: babel.Locale) -> str:
    """Returns OnlineFinder's language tag from the locale and if exits, the tag
...
def get_engine_locale(onlinefinder_locale: str, engine_locales: dict[str, str], default: str | None = None) -> str | None:
    """Return engine's language (aka locale) string that best fits to argument
    ``onlinefinder_locale``.

    Argument ``engine_locales`` is a python dict that maps *OnlineFinder locales* to
    corresponding *engine locales*::

      <engine>: {
          # OnlineFinder string : engine-string
          'ca-ES'          : 'ca_ES',
          'fr-BE'          : 'fr_BE',
          'fr-CA'          : 'fr_CA',
          'fr-CH'          : 'fr_CH',
    ...
    .. hint::
       ...
       The *OnlineFinder locale* string has to be known by babel!
    ...
    engine_locale = engine_locales.get(onlinefinder_locale)
    ...
    try:
        locale = babel.Locale.parse(onlinefinder_locale, sep='-')
    except babel.core.UnknownLocaleError:
        try:
            locale = babel.Locale.parse(onlinefinder_locale.split('-')[0])
        except babel.core.UnknownLocaleError:
            return default

    onlinefinder_lang = language_tag(locale)
    engine_locale = engine_locales.get(onlinefinder_lang)
    if engine_locale is not None:
        # There was a 1:1 mapping (e.g. "zh-HK --> zh_Hant" or "zh-CN --> zh_Hans")
        return engine_locale

    # OnlineFinder's selected locale is not supported by the engine ..
    ...
    if locale.territory:
        ...
        for official_language in babel.languages.get_official_languages(locale.territory, de_facto=True):
            onlinefinder_locale = official_language + '-' + locale.territory
            engine_locale = engine_locales.get(onlinefinder_locale)
            if engine_locale is not None:
                return engine_locale
    ...
    # Now lets have a look if the onlinefinder_lang (the language selected by the
    # user) is a official language in other territories.  If so, check if
    # engine does support the onlinefinder_lang in this other territory.
    ...
    if locale.language:
        ...
        for territory, langs in babel.core.get_global("territory_languages").items():
            _lang = langs.get(onlinefinder_lang)
        ...
        if terr_lang_dict.get(territory):
            onlinefinder_locale = locale.language + '-' + territory
            engine_locale = engine_locales.get(onlinefinder_locale)
            if engine_locale is not None:
                return engine_locale
...
def match_locale(onlinefinder_locale: str, locale_tag_list: list[str], fallback: str | None = None) -> str | None:
    """Return tag from ``locale_tag_list`` that best fits to ``onlinefinder_locale``.

    :param str onlinefinder_locale: OnlineFinder's internal representation of locale (de,
    ...
    .. hint::
       ...
       The *OnlineFinder locale* string and the members of ``locale_tag_list`` has to
       be known by babel!  The :py:obj:`ADDITIONAL_TRANSLATIONS` are used in the
       UI and are not known by babel --> will be ignored.
    """

    # onlinefinder_locale = 'es'
    # locale_tag_list = ['es-AR', 'es-ES', 'es-MX']

    if not onlinefinder_locale:
        return fallback

    locale = get_locale(onlinefinder_locale)
    if locale is None:
        return fallback

    # normalize to a OnlineFinder locale that can be passed to get_engine_locale

    onlinefinder_locale = language_tag(locale)
    if locale.territory:
        onlinefinder_locale = region_tag(locale)
    ...
    return get_engine_locale(onlinefinder_locale, engine_locales, default=fallback)
...
def build_engine_locales(tag_list: list[str]) -> dict[str, str]:
    ...
    of OnlineFinder:
      ...
      OnlineFinder only distinguishes between **search languages** and **search
      regions**, by adding the *script-tags*, languages with *script-tags* can
      be assigned to the **regions** that OnlineFinder supports.
