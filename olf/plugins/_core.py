...
if t.TYPE_CHECKING:
    from olf.search import SearchWithPlugins
    from olf.result_types import Result, EngineResults, LegacyResult  # pyright: ignore[reportPrivateLocalImportUsage]
    import flask

log: logging.Logger = logging.getLogger("olf.plugins")
...
class Plugin(abc.ABC):
    ...
    def on_result(self, request: SXNG_Request, search: "SearchWithPlugins", result: "Result") -> bool:
        ...
        .. hint::
           ...
           If :py:obj:`Result.url <olf.result_types._base.Result.url>` is modified,
           :py:obj:`Result.parsed_url <olf.result_types._base.Result.parsed_url>` must
    ...
    def post_search(
        ...
        :py:obj:`Result <olf.result_types._base.Result>` objects to be added to the
        final result list."""
        return
...
class PluginStorage:
    """A storage for managing the *plugins* of OnlineFinder."""
    ...
    def load_settings(self, cfg: dict[str, dict[str, t.Any]]):
        """Load plugins configured in OnlineFinder's settings :ref:`settings
        plugins`."""
    ...
    def post_search(self, request: SXNG_Request, search: "SearchWithPlugins") -> None:
        ...
        <olf.results.ResultContainer`> with result items from plugins listed
        ...
        for plugin in [p for p in self.plugin_list if p.id in search.user_plugins]:
            ...
            # In case of *plugins* prefix ``plugin:`` is set, see olf.result_types.Result
