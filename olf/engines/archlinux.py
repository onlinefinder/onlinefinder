...
from olf.utils import extract_text, eval_xpath_list, eval_xpath_getindex, onlinefinder_useragent
...
def request(query, params):
    ...
    sxng_lang = params['onlinefinder_locale'].split('-')[0]
    ...
    if netloc == main_wiki:
        ...
        # - https://github.com/onlinefinder/onlinefinder/issues/4646#issuecomment-2817848019
        params['headers']['User-Agent'] = onlinefinder_useragent()
...
def response(resp):
    ...
    sxng_lang = resp.search_params['onlinefinder_locale'].split('-')[0]
    netloc: str = traits.custom['wiki_netloc'].get(sxng_lang, main_wiki)  # type: ignore
    base_url = 'https://' + netloc + '/index.php?'
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    <olf.enginelib.traits.EngineTraits.custom>` (``wiki_netloc``).  Depending
