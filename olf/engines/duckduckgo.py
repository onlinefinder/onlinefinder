...
from olf.utils import (
    eval_xpath,
    eval_xpath_getindex,
    extr,
    extract_text,
)
from olf.network import get  # see https://github.com/onlinefinder/onlinefinder/issues/762
from olf.enginelib.traits import EngineTraits
from olf.enginelib import EngineCache
from olf.exceptions import SearxEngineCaptchaException
...
def get_vqd(query: str, region: str, force_request: bool = False) -> str:
    ...
    If OnlineFinder cannot determine a ``vqd`` value, then no request should go out
    to DDG.

    .. attention::

       A request with a wrong ``vqd`` value leads to DDG temporarily putting
       OnlineFinder's IP on a block list.
...
def get_ddg_lang(eng_traits: EngineTraits, sxng_locale, default='en_US'):
    """Get DuckDuckGo's language identifier from OnlineFinder's locale.
    ...
    .. code: python
       ...
       eng_region = traits.get_region(params['onlinefinder_locale'], traits.all_locale)
       eng_lang = get_ddg_lang(traits, params['onlinefinder_locale'])

    It might confuse, but the ``l`` value of the cookie is what OnlineFinder calls
    the *region*:
...
def request(query, params):
    ...
    eng_region: str = traits.get_region(params['onlinefinder_locale'], traits.all_locale)  # type: ignore
    ...
    if params['pageno'] == 1:
        ...
        if vqd:
            ...
            # reputation of the OnlineFinder IP and DDG starts to activate CAPTCHAs.
            params["url"] = None
            return

        if params['onlinefinder_locale'].startswith("zh"):
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    OnlineFinder's ``all`` locale maps DuckDuckGo's "Alle regions" (``wt-wt``).
    DuckDuckGo's language "Browsers preferred language" (``wt_WT``) makes no
    sense in a OnlineFinder request since OnlineFinder's ``all`` will not add a
