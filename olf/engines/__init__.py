...
from olf import logger, settings
...
def check_engine_module(module: types.ModuleType):
    ...
    # network: https://github.com/onlinefinder/onlinefinder/issues/762#issuecomment-1605323861
...
def load_engine(engine_data: dict[str, t.Any]) -> "Engine | types.ModuleType | None":
    ...
    from olf.enginelib.traits import EngineTraitsMap
...
def set_loggers(engine: "Engine|types.ModuleType", engine_name: str):
    ...
    for module_name, module in modules.items():
        if (
            module_name.startswith("olf.engines")
            and module_name != "olf.engines.__init__"
