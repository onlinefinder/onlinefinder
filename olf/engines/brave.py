...
Brave uses two-digit tags for the regions like ``ca`` while OnlineFinder deals with
locales.  To get a mapping, all *officiat de-facto* languages of the Brave
region are mapped to regions in OnlineFinder (see :py:obj:`babel
...
In the :py:obj:`EngineTraits object <olf.enginelib.traits.EngineTraits>` the
...
in OnlineFinder, the spellchecking is disabled by default.
"""
...
def request(query: str, params: dict[str, t.Any]) -> None:
    ...
    engine_region = traits.get_region(params["onlinefinder_locale"], "all")
    params["cookies"]["country"] = engine_region.split("-")[-1].lower()  # type: ignore

    ui_lang = locales.get_engine_locale(params["onlinefinder_locale"], traits.custom["ui_lang"], "en-us")
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    from olf.locales import region_tag, language_tag
