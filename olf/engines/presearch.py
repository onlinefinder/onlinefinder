...
video streams OnlineFinder can't use the video template for this and if OnlineFinder can't
...
from olf.network import get
from olf.utils import gen_useragent, html_to_text, parse_duration_string
...
def _get_request_id(query, params):
    ...
    if params['onlinefinder_locale'] != 'all':
        l = locales.get_locale(params['onlinefinder_locale'])

        # Presearch narrows down the search by region.  In OnlineFinder when the user
        # does not set a region (e.g. 'en-CA' / canada) we cannot hand over a region.

        # We could possibly use olf.locales.get_official_locales to determine
...
def response(resp):
    ...
    if search_type == 'search':
        ...
        # a video and not to a video stream --> OnlineFinder can't use the video template.
