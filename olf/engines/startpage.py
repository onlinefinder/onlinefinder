...
from olf.utils import extr, extract_text, eval_xpath, gen_useragent, html_to_text, humanize_bytes, remove_pua_from_str
...
from olf.locales import region_tag
...
def get_sc_code(onlinefinder_locale, params):
    ...
    if onlinefinder_locale == 'all':
        onlinefinder_locale = 'en-US'
    ...
    try:
        ...
        logger.debug("suspend startpage API --> https://github.com/onlinefinder/onlinefinder/pull/695")
        raise SearxEngineCaptchaException(
...
def request(query, params):
    ...
    """
    engine_region = traits.get_region(params['onlinefinder_locale'], 'en-US')
    engine_language = traits.get_language(params['onlinefinder_locale'], 'en')
    ...
    args = {
        ...
        'cat': startpage_categ,
        't': 'device',
        'sc': get_sc_code(params['onlinefinder_locale'], params),  # hint: this func needs HTTP headers
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    skip_eng_tags = {
        'english_uk',  # OnlineFinder lang 'en' already maps to 'english'
    }
