...
To support Wikipedia's LanguageConverter_, a OnlineFinder request to Wikipedia uses
:py:obj:`get_wiki_params` and :py:obj:`wiki_lc_locale_variants' in the
:py:obj:`fetch_wikimedia_traits` function.

To test in OnlineFinder, query for ``!wp 出租車`` with each of the available Chinese
options:
...
.. _PR-2554: https://github.com/olf/olf/pull/2554
...
from olf import utils
from olf import network as _network
...
def request(query, params):
    ...
    _eng_tag, wiki_netloc = get_wiki_params(params['onlinefinder_locale'], traits)
...
def fetch_wikimedia_traits(engine_traits: EngineTraits):
    ...
    :py:obj:`list_of_wikipedias` are supported by OnlineFinder locales, only those
    known from :py:obj:`olf.locales.LOCALE_NAMES` or those with a minimal
    :py:obj:`editing depth <wikipedia_article_depth>`.

    The location of the Wikipedia address of a language is mapped in a
    :py:obj:`custom field <olf.enginelib.traits.EngineTraits.custom>`
