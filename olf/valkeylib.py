...
def purge_by_prefix(client, prefix: str = "OnlineFinder_"):
    """Purge all keys with ``prefix`` from database.

    Queries all keys in the database by the given prefix and set expire time to
    zero.  The default prefix will drop all keys which has been set by OnlineFinder
    (drops OnlineFinder schema entirely from database).
    ...
    :param prefix: prefix of the key to delete (default: ``OnlineFinder_``)
    :type name: str
...
def incr_counter(client, name: str, limit: int = 0, expire: int = 0):
    ...
    If counter with valkey key ``OnlineFinder_counter_<name>`` does not exists it is
    ...
    *secret hash* of the value from argument ``name`` (see
    ...
    name = "OnlineFinder_counter_" + secret_hash(name)
    c = script(args=[limit, expire], keys=[name])
    return c


def drop_counter(client, name):
    """Drop counter with valkey key ``OnlineFinder_counter_<name>``
...
def incr_sliding_window(client, name: str, duration: int):
    ...
    *secret hash* of the value from argument ``name`` (see
    ...
    to implement a sliding window for the valkey key ``OnlineFinder_counter_<name>``
