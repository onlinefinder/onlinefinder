...
from olf.exceptions import (
    SearxEngineAccessDeniedException,
    SearxEngineCaptchaException,
    SearxEngineTooManyRequestsException,
)
from olf.metrics.error_recorder import count_error
...
class OnlineProcessor(EngineProcessor):
    ...
    def init_network_in_thread(self, start_time: float, timeout_limit: float):
        ...
        olf.network.set_timeout_for_thread(timeout_limit, start_time=start_time)
        # reset the HTTP total time
        olf.network.reset_time_for_thread()
        # set the network
        olf.network.set_context_network_name(self.engine.name)
    ...
    def search(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        try:
            ...
            self.logger.error("SSLError {}, verify={}".format(e, olf.network.get_network(self.engine.name).verify))
