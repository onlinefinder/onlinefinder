...
import olf.plugins
from olf.engines import load_engines
from olf.external_bang import get_bang_url
from olf.metrics import initialize as initialize_metrics, counter_inc
from olf.network import initialize as initialize_network, check_network_configuration
from olf.results import ResultContainer
from olf.search.checker import initialize as initialize_checker
from olf.search.processors import PROCESSORS
from olf.search.processors.abstract import RequestParams
...
class Search:
    ...
    def search_answerers(self):
        ...
        results = olf.answerers.STORAGE.ask(self.search_query.query)
...
class SearchWithPlugins(Search):
    ...
    def _on_result(self, result):
        return olf.plugins.STORAGE.on_result(self.request, self, result)

    def search(self) -> ResultContainer:

        if olf.plugins.STORAGE.pre_search(self.request, self):
            super().search()

        olf.plugins.STORAGE.post_search(self.request, self)
