...
import olf.search
import olf.search.checker
from olf.search import PROCESSORS
from olf.engines import engine_shortcuts
...
def iter_processor(engine_name_list):
    if len(engine_name_list) > 0:
        ...
        for name, processor in olf.search.PROCESSORS.items():
            yield name, processor
...
def run(engine_name_list, verbose):
    ...
    for name, processor in iter_processor(engine_name_list):
        ...
        checker = olf.search.checker.Checker(processor)
        checker.run()
...
# call by setup.py
def main():
    parser = argparse.ArgumentParser(description='Check OnlineFinder engines.')
    parser.add_argument(
        'engine_name_list',
        metavar='engine name',
        type=str,
