...
from olf import settings
from olf.data import USER_AGENTS, data_dir
from olf.version import VERSION_TAG
from olf.sxng_locales import sxng_locales
from olf.exceptions import SearxXPathSyntaxException, SearxEngineXPathException
from olf import logger
...
"""Type alias used by :py:obj:`olf.utils.get_xpath`,
:py:obj:`olf.utils.eval_xpath` and other XPath selectors."""
...
SEARCH_LANGUAGE_CODES = frozenset([onlinefinder_locale[0].split('-')[0] for onlinefinder_locale in sxng_locales])
"""Languages supported by most onlinefinder engines (:py:obj:`olf.sxng_locales.sxng_locales`)."""
...
def onlinefinder_useragent() -> str:
    """Return the OnlineFinder User Agent"""
    return f"OnlineFinder/{VERSION_TAG} {settings['outgoing']['useragent_suffix']}".strip()


def gen_useragent(os_string: str | None = None) -> str:
    """Return a random browser User Agent

    See olf/data/useragents.json
    """
...
def extract_url(xpath_results: list[ElementType] | ElementType | str | Number | bool | None, base_url: str) -> str:
    ...
    Example:
        >>> def f(s, search_url):
        >>>    return olf.utils.extract_url(html.fromstring(s), search_url)
        ...
        >>> olf.utils.extract_url([], 'https://example.com')
        raise ValueError
...
def eval_xpath_list(element: ElementType, xpath_spec: XPathSpecType, min_len: int | None = None) -> list[t.Any]:
    """Same as :py:obj:`olf.utils.eval_xpath`, but additionally ensures the
...
def eval_xpath_getindex(
    ...
    """Same as :py:obj:`olf.utils.eval_xpath_list`, but returns item on
    position ``index`` from the list (index starts with ``0``).

    The exceptions known from :py:obj:`olf.utils.eval_xpath` are thrown. If a
...
def detect_language(text: str, threshold: float = 0.3, only_search_languages: bool = False) -> str | None:
    ...
    :param bool only_search_languages: If ``True``, returns only supported
        OnlineFinder search languages.  see :py:obj:`olf.languages`
    ...
    is harmonized with the OnlineFinder's language (locale) model.  General
    conditions of OnlineFinder's locale model are:

    a. OnlineFinder's locale of a query is passed to the
       :py:obj:`olf.locales.get_engine_locale` to get a language and/or region
       code that is used by an engine.

    b. Most of OnlineFinder's engines do not support all the languages from `language
       identification model`_ and there is also a discrepancy in the ISO-639-3
       (fasttext) and ISO-639-2 (OnlineFinder)handling.  Further more, in OnlineFinder the
       locales like ``zh-TH`` (``zh-CN``) are mapped to ``zh_Hant``
       (``zh_Hans``) while the `language identification model`_ reduce both to
       ``zh``.

    .. _a fork: https://github.com/onlinefinder/fasttext-predict
