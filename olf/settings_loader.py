...
includes the configuration of the (:ref:`OnlineFinder appl <onlinefinder settings.yml>`)
server. The default configuration for the application server is loaded from the
:origin:`DEFAULT_SETTINGS_FILE <olf/settings.yml>`.  This default
configuration can be completely replaced or :ref:`customized individually
<use_default_settings.yml>` and the ``ONLINEFINDER_SETTINGS_PATH`` environment
...
- By default, local configurations are expected in folder ``/etc/onlinefinder`` from
  where applications can load them with the :py:obj:`get_yaml_cfg` function.

- By default, customized :ref:`OnlineFinder appl <onlinefinder settings.yml>` settings are
...
from olf.exceptions import SearxSettingsException
...
"""The :origin:`olf/settings.yml` file with all the default settings."""
...
def get_yaml_cfg(file_name: str | Path) -> SettingsType:
    ...
    - in the ``olf`` folder of the OnlineFinder installation
    """
...
def get_user_cfg_folder() -> Path | None:
    ...
    1. If the ``ONLINEFINDER_SETTINGS_PATH`` environment is set and points to a
       folder (e.g. ``/etc/mysxng/``), all local configurations are expected in
       this folder.  The settings of the :ref:`OnlineFinder appl <onlinefinder
       settings.yml>` then expected in ``settings.yml``
       (e.g. ``/etc/mysxng/settings.yml``).

    2. If the ``ONLINEFINDER_SETTINGS_PATH`` environment is set and points to a file
       (e.g. ``/etc/mysxng/myinstance.yml``), this file contains the settings of
       the :ref:`OnlineFinder appl <onlinefinder settings.yml>` and the folder
       (e.g. ``/etc/mysxng/``) is used for all other configurations.

       This type (``ONLINEFINDER_SETTINGS_PATH`` points to a file) is suitable for
       use cases in which different profiles of the :ref:`OnlineFinder appl <onlinefinder
       settings.yml>` are to be managed, such as in test scenarios.

    3. If folder ``/etc/onlinefinder`` exists, it is used.

    In case none of the above path exists, ``None`` is returned.  In case of
    environment ``ONLINEFINDER_SETTINGS_PATH`` is set, but the (folder or file) does
    not exists, a :py:obj:`EnvironmentError` is raised.

    """

    folder = None
    settings_path = os.environ.get("ONLINEFINDER_SETTINGS_PATH")

    # Disable default /etc/onlinefinder is intended exclusively for internal testing purposes
    # and is therefore not documented!
    disable_etc = os.environ.get('ONLINEFINDER_DISABLE_ETC_SETTINGS', '').lower() in ('1', 'true')
...
def load_settings(load_user_settings: bool = True) -> tuple[SettingsType, str]:
    """Function for loading the settings of the OnlineFinder application
    (:ref:`settings.yml <onlinefinder settings.yml>`)."""
    ...
    settings_yml = os.environ.get("ONLINEFINDER_SETTINGS_PATH")
    if settings_yml and Path(settings_yml).is_file():
        # see get_user_cfg_folder() --> ONLINEFINDER_SETTINGS_PATH points to a file
        settings_yml = Path(settings_yml).name
    else:
        # see get_user_cfg_folder() --> ONLINEFINDER_SETTINGS_PATH points to a folder
