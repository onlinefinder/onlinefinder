...
"""Implementations of the framework for the OnlineFinder engines.

- :py:obj:`olf.enginelib.EngineCache`
- :py:obj:`olf.enginelib.Engine`
- :py:obj:`olf.enginelib.traits`
...
an example in which the command line is called in the development environment::
  ...
  (py3) python -m olf.enginelib --help
...
.. hint::
   ...
   - move implementations of the :ref:`olf.engines loader` to a new module in
     the :py:obj:`olf.enginelib` namespace.
...
if t.TYPE_CHECKING:
    from olf.enginelib import traits
    from olf.enginelib.traits import EngineTraits
    from olf.extended_types import SXNG_Response
    from olf.result_types import EngineResults
    from olf.search.processors import OfflineParamTypes, OnlineParamTypes
...
"""Global :py:obj:`olf.cache.ExpireCacheSQLite` instance where the cached
...
class EngineCache:
    ...
    <olf.cache.ExpireCacheCfg.MAXHOLD_TIME>`).  This class is a wrapper around
    :py:obj:`ENGINES_CACHE` (:py:obj:`ExpireCacheSQLite
    <olf.cache.ExpireCacheSQLite>`).

    In the :origin:`olf/engines/demo_offline.py` engine you can find an
    exemplary implementation of such a cache other examples are implemented
    in:

    - :origin:`olf/engines/radio_browser.py`
    - :origin:`olf/engines/soundcloud.py`
    - :origin:`olf/engines/startpage.py`

    .. code: python

       from olf.enginelib import EngineCache
       CACHE: EngineCache
    ...
    show cache state::
        ...
        $ ./manage pyenv.cmd bash --norc --noprofile
        (py3) python -m olf.enginelib cache state
...
class Engine(abc.ABC):  # pylint: disable=too-few-public-methods
    ...
    engine_type: str
    """Type of the engine (:ref:`olf.search.processors`)"""
    ...
    name: str
    """Name that will be used across OnlineFinder to define this engine.  In settings, on
    the result page .."""

    engine: str
    """Name of the python file used to handle requests and responses to and from
    this search engine (file name from :origin:`olf/engines` without
    ``.py``)."""
