...
their mapping from OnlineFinder's representation to the representation in the origin
...
import typing as t
import pathlib
from olf import locales
from olf.data import data_dir, ENGINE_TRAITS
...
class EngineTraits:
    ...
    """Maps OnlineFinder's internal representation of a region to the one of the engine.

    OnlineFinder's internal representation can be parsed by babel and the value is
    send to the engine:
    ...
    .. code:: python
       ...
       for key, egnine_region regions.items():
          onlinefinder_region = babel.Locale.parse(key, sep='-')
          ...
    """

    languages: dict[str, str] = dataclasses.field(default_factory=dict)
    """Maps OnlineFinder's internal representation of a language to the one of the engine.
    ...
    .. code:: python
       ...
       for key, egnine_lang in languages.items():
          onlinefinder_lang = babel.Locale.parse(key)
          ...
    """

    all_locale: str | None = None
    """To which locale value OnlineFinder's ``all`` language is mapped (shown a "Default
    language").
    """
    ...
    """A place to store engine's custom traits, not related to the OnlineFinder core.
    """

    def get_language(self, onlinefinder_locale: str, default: t.Any = None):
        """Return engine's language string that *best fits* to OnlineFinder's locale.

        :param onlinefinder_locale: OnlineFinder's internal representation of locale
          selected by the user.

        :param default: engine's default language

        The *best fits* rules are implemented in
        :py:obj:`olf.locales.get_engine_locale`.  Except for the special value ``all``
        which is determined from :py:obj:`EngineTraits.all_locale`.
        """
        if onlinefinder_locale == 'all' and self.all_locale is not None:
            return self.all_locale
        return locales.get_engine_locale(onlinefinder_locale, self.languages, default=default)

    def get_region(self, onlinefinder_locale: str, default: t.Any = None) -> t.Any:
        """Return engine's region string that best fits to OnlineFinder's locale.
        ...
        if onlinefinder_locale == 'all' and self.all_locale is not None:
            return self.all_locale
        return locales.get_engine_locale(onlinefinder_locale, self.regions, default=default)

    def is_locale_supported(self, onlinefinder_locale: str) -> bool:
        """A *locale* (OnlineFinder's internal representation) is considered to be
        ...
        if self.data_type == 'traits_v1':
            return bool(self.get_region(onlinefinder_locale) or self.get_language(onlinefinder_locale))
    ...
    def set_traits(self, engine: "Engine | types.ModuleType"):
        ...
        :param engine: engine instance build by :py:func:`olf.engines.load_engine`
        """
...
class EngineTraitsMap(dict[str, EngineTraits]):
    ...
    def fetch_traits(cls, log: t.Callable[[str], None]) -> 'EngineTraitsMap':
        from olf import engines  # pylint: disable=cyclic-import, import-outside-toplevel
        ...
        for engine_name in names:
            ...
            if traits is not None:
                log("%-20s: OnlineFinder languages --> %s " % (engine_name, len(traits.languages)))
                log("%-20s: OnlineFinder regions   --> %s" % (engine_name, len(traits.regions)))
