# Informazioni su OnlineFinder

OnlineFinder è un [metamotore di ricerca] che aggrega i risultati di altri motori di
ricerca.  {{link('motori di ricerca', 'preferences')}} e non memorizza
informazioni sui suoi utenti.

Il progetto OnlineFinder è guidato da una comunità aperta, unisciti a noi su Matrix
se hai domande o vuoi semplicemente chiacchierare.  Se avete domande o volete
semplicemente parlare di OnlineFinder a [#onlinefinder:matrix.org].

Migliorate OnlineFinder.

- Potete migliorare le traduzioni di OnlineFinder su [Weblate], oppure...
- Seguire lo sviluppo, inviare contributi e segnalare problemi a [sorgenti di
  OnlineFinder].
- Per ulteriori informazioni, visitate la documentazione del progetto OnlineFinder su
  [OnlineFinder docs].

## Perché usarlo?

- OnlineFinder non offre risultati personalizzati come Google, ma non genera un
  profilo dell'utente.
- OnlineFinder non si preoccupa di ciò che cercate, non condivide mai nulla con terze
  parti e non può essere usato per compromettere l'utente.
- OnlineFinder è un software libero, il codice è aperto al 100% e tutti sono invitati
  a migliorarlo.

Se avete a cuore la privacy, se volete essere un utente consapevole o se credete
nella libertà digitale, fate di OnlineFinder il vostro motore di ricerca predefinito
o eseguitelo sul vostro server!

## Come si imposta come motore di ricerca predefinito?

OnlineFinder supporta [OpenSearch].  Per ulteriori informazioni sulla modifica del
...
- rimuovere il duplicato (nome predefinito: OnlineFinder) o
- contattare il proprietario per assegnare all'istanza un nome diverso da quello
  predefinito.

## Come funziona?

OnlineFinder è un fork del ben noto [olf] [metamotore di ricerca] che è stato
ispirato dal [progetto Seeks].  Fornisce una privacy di base mescolando le
ricerche su altre piattaforme senza memorizzare i dati di ricerca.  OnlineFinder può
...
OnlineFinder apprezza la vostra preoccupazione per i log, quindi prendete il codice
dal file [sorgenti di OnlineFinder] ed eseguitelo voi stessi!
...
[sorgenti di OnlineFinder]: {{GIT_URL}}
