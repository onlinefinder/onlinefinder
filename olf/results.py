...
from olf import logger as log
import olf.engines
from olf.metrics import histogram_observe, counter_add
from olf.result_types import Result, LegacyResult, MainResult
from olf.result_types.answer import AnswerSet, BaseAnswer
...
def calculate_score(
    ...
    for result_engine in result['engines']:
        if hasattr(olf.engines.engines.get(result_engine), 'weight'):
            weight *= float(olf.engines.engines[result_engine].weight)
...
class ResultContainer:
    ...
    def extend(
        ...
        if engine_name in olf.engines.engines:
            eng = olf.engines.engines[engine_name]
    ...
    def get_ordered_results(self) -> list[MainResult | LegacyResult]:
        ...
        for res in results:
            ...
            engine = olf.engines.engines.get(res.engine or "")
            if engine:
    ...
    def add_unresponsive_engine(self, engine_name: str, error_type: str, suspended: bool = False):
        with self._lock:
            if self._closed:
                ...
                return
            if olf.engines.engines[engine_name].display_error_messages:
...
def merge_two_infoboxes(origin: LegacyResult, other: LegacyResult):
    ...
    # pylint: disable=too-many-branches
    weight1 = getattr(olf.engines.engines[origin.engine], "weight", 1)
    weight2 = getattr(olf.engines.engines[other.engine], "weight", 1)
