...
from olf.network import get as http_get, post as http_post  # pyright: ignore[reportUnknownVariableType]
from olf.exceptions import SearxEngineResponseException
from olf.utils import extr, gen_useragent
...
def google_complete(query: str, sxng_locale: str) -> list[str]:
    ...
    (:py:obj:`olf.engines.google.get_google_info`) by using the async REST
    API::
    ...
    google_info: dict[str, t.Any] = google.get_google_info({'onlinefinder_locale': sxng_locale}, engines['google'].traits)
...
def sogou(query: str, _sxng_locale: str) -> list[str]:
    ...
    resp = get(base_url + urlencode({'m': 'onlinefinder', 'key': query}))
    results: list[str] = []
