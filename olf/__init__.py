...
def init_settings():
    ...
    ``logger`` from ``ONLINEFINDER_SETTINGS_PATH``.
    """

    # pylint: disable=import-outside-toplevel
    from olf import settings_loader
    from olf.settings_defaults import SCHEMA, apply_schema
    ...
    if settings['server']['public_instance']:
        logger.warning(
            ...
            "see https://docs.onlinefinder.org/admin/olf.limiter.html"
        )
...
def _logging_config_debug():
    ...
    log_level = os.environ.get('ONLINEFINDER_DEBUG_LOG_LEVEL', 'DEBUG')
