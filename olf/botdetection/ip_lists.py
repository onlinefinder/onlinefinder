...
.. code:: toml
   ...
   pass_ip = [
     '167.235.158.251', # IPv4 of check.olf.space
...
ONLINEFINDER_ORG = [
    # https://github.com/onlinefinder/onlinefinder/pull/2484#issuecomment-1576639195
    '167.235.158.251',  # IPv4 check.olf.space
    '2a01:04f8:1c1c:8fc2::/64',  # IPv6 check.olf.space
]
"""Passlist of IPs from the OnlineFinder organization, e.g. `check.olf.space`."""
...
def pass_ip(real_ip: IPv4Address | IPv6Address, cfg: config.Config) -> Tuple[bool, str]:
    ...
    if cfg.get('botdetection.ip_lists.pass_onlinefinder_org', default=True):
        for net in ONLINEFINDER_ORG:
            net = ip_network(net, strict=False)
            if real_ip.version == net.version and real_ip in net:
                return True, f"IP matches {net.compressed} in ONLINEFINDER_ORG list."
