...
def get_git_version() -> tuple[str, str, str]:
    ...
    # add "+dirty" suffix if there are uncommitted changes except olf/settings.yml
    try:
        subprocess_run("git diff --quiet -- . ':!olf/settings.yml' ':!utils/brand.env'")
...
# this file is generated automatically by olf/version.py
...
"""
        ...
        # usage: eval "$(python -m olf.version)"
        shell_code = f"""
VERSION_STRING="{VERSION_STRING}"
VERSION_TAG="{VERSION_TAG}"
DOCKER_TAG="{DOCKER_TAG}"
