...
from olf.webutils import new_hmac, is_hmac_of
...
def _initial_resolver_map():
    ...
    if name:
        ...
        if func:
            d = {name: f"olf.favicons.resolvers.{func.__name__}"}
    return d
...
class FaviconProxyConfig(msgspec.Struct):
    ...
    :py:obj:`olf.favicons.resolver` are available by default."""
...
def favicon_proxy():
    """REST API of OnlineFinder's favicon proxy service
...
def favicon_url(authority: str) -> str:
    """Function to generate the image URL used for favicons in OnlineFinder's result
