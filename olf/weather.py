...
from olf import network
from olf.cache import ExpireCache, ExpireCacheCfg
from olf.extended_types import sxng_request
from olf.wikidata_units import convert_to_si, convert_from_si
...
def _get_sxng_locale_tag() -> str:
    ...
    #
    # In principle, OnlineFinder only has two possible parameters for determining
    ...
    # probably never be available in OnlineFinder (privacy critical).
    #
    ...
    from olf import query
    from olf.preferences import ClientPref
...
class WindSpeed(msgspec.Struct, kw_only=True):
    ...
    .. hint::
       ...
       Working with unit ``Bft`` (:py:obj:`olf.wikidata_units.Beaufort`) will
