...
that OnlineFinder passes through requests from bots and is thus classified as a bot
itself.  As a result, the OnlineFinder engine then receives a CAPTCHA or is blocked
by the search engine (the origin) in some other way.

To avoid blocking, the requests from bots to OnlineFinder must also be blocked, this
...
.. _NGINX RequestHeader:
    https://docs.onlinefinder.org/admin/installation-nginx.html#nginx-s-onlinefinder-site
.. _Apache RequestHeader:
    https://docs.onlinefinder.org/admin/installation-apache.html#apache-s-onlinefinder-site
...
file ``/etc/onlinefinder/limiter.toml``.  The defaults are shown in limiter.toml_ /
...
from olf import botdetection
from olf.extended_types import SXNG_Request, sxng_request
from olf.botdetection import (
    config,
    http_accept,
...
def get_cfg() -> config.Config:
    """Returns OnlineFinder's global limiter configuration."""
    global CFG  # pylint: disable=global-statement

    if CFG is None:
        from . import settings_loader  # pylint: disable=import-outside-toplevel

        cfg_file = (settings_loader.get_user_cfg_folder() or Path("/etc/onlinefinder")) / "limiter.toml"
        CFG = config.Config.from_toml(LIMITER_CFG_SCHEMA, cfg_file, olf.compat.LIMITER_CFG_DEPRECATED)
        olf.compat.limiter_fix_cfg(CFG, cfg_file)
...
def initialize(app: flask.Flask, settings):
    ...
    if not valkey_client:
        logger.error(
            ...
            "https://docs.onlinefinder.org/admin/olf.limiter.html"
        )
