...
from olf import infopage
from olf import limiter
from olf.botdetection import link_token, ProxyFix

from olf.data import ENGINE_DESCRIPTIONS
from olf.result_types import Answer
from olf.settings_defaults import OUTPUT_FORMATS
from olf.settings_loader import DEFAULT_SETTINGS_FILE
from olf.exceptions import SearxParameterException
from olf.engines import (
    DEFAULT_CATEGORY,
    categories,
    engines,
    engine_shortcuts,
...
from olf import webutils
from olf.webutils import (
    highlight_content,
...
from olf.webadapter import (
    get_search_query_from_webapp,
    get_selected_categories,
    parse_lang,
)
from olf.utils import gen_useragent, dict_subset
from olf.version import VERSION_STRING, GIT_URL, GIT_BRANCH
from olf.query import RawTextQuery
from olf.plugins.oa_doi_rewrite import get_doi_resolver
from olf.preferences import (
    Preferences,
    ClientPref,
    ValidationException,
)
...
from olf.metrics import get_engines_stats, get_engine_errors, get_reliabilities, histogram, counter, openmetrics
from olf.flaskfix import patch_application

from olf.locales import (
    LOCALE_BEST_MATCH,
    LOCALE_NAMES,
    RTL_LOCALES,
    localeselector,
...
# renaming names from olf imports ...
from olf.autocomplete import search_autocomplete, backends as autocomplete_backends
from olf import favicons

from olf.valkeydb import initialize as valkey_initialize
from olf.sxng_locales import sxng_locales
import olf.search
from olf.network import stream as http_stream, set_context_network_name
from olf.search.checker import get_result as checker_get_result
...
def pre_request():
    ...
    preferences = Preferences(themes, list(categories.keys()), engines, olf.plugins.STORAGE, client_pref)
    ...
    for plugin in olf.plugins.STORAGE:
...
def index_error(output_format: str, error_message: str):
    ...
    if output_format == 'csv':
        ...
        cont_disp = 'attachment;Filename=olf.csv'
...
def search():
    ...
    try:
        ...
        search_obj = olf.search.SearchWithPlugins(search_query, sxng_request, sxng_request.user_plugins)
...
def autocompleter():
    ...
    for obj in olf.answerers.STORAGE.ask(sug_prefix):
    ...
    if len(raw_text_query.autocomplete_list) == 0 and len(sug_prefix) > 0:
        ...
        # get OnlineFinder's locale and autocomplete backend from cookie
    ...
    if sxng_request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        # the suggestion request comes from the olf search form
...
def image_proxy():
    ...
    try:
        ...
        if resp and not forward_resp:
            ...
            # we make sure to close the response between onlinefinder and the HTTP server
            try:
                resp.close()
            except httpx.HTTPError:
...
def engine_descriptions():
    ...
    for engine_name, engine_mod in engines.items():
        ...
        if descr is not None:
            result[engine_name] = [descr, "OnlineFinder config"]
    ...
    return jsonify(
        {
            ...
            'limiter': {
                ...
                'botdetection.ip_lists.pass_onlinefinder_org': _limiter_cfg.get('botdetection.ip_lists.pass_onlinefinder_org'),
            },
...
def run():
    ...
    This run method is only called when OnlineFinder is started via ``__main__``::
    ...
    command line script’s run support::
        ...
        flask --app olf.webapp run --debug --reload --host 127.0.0.1 --port 8888
...
def init():
    ...
    if olf.sxng_debug or app.debug:
        app.debug = True
        olf.sxng_debug = True
    ...
    locales_initialize()
    valkey_initialize()
    olf.plugins.initialize(app)

    metrics: bool = get_setting("general.enable_metrics")  # type: ignore
    olf.search.initialize(enable_checker=True, check_network=True, enable_metrics=metrics)
