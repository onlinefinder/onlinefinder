...
from olf.utils import load_module
from olf.result_types.answer import BaseAnswer


_default = pathlib.Path(__file__).parent
log: logging.Logger = logging.getLogger("olf.answerers")
...
class AnswerStorage(dict):  # type: ignore
    """A storage for managing the *answerers* of OnlineFinder.  With the
    ...
    *answerers* and receives a list of the results."""
    ...
    def load_builtins(self):
        ...
        :origin:`olf/answerers`.  The python modules are wrapped by
        ...
        for f in _default.iterdir():
            ...
            if f.is_file() and f.suffix == ".py":
                self.register_by_fqn(f"olf.answerers.{f.stem}.SXNGAnswerer")
                continue
            ...
            if f.is_dir() and (f / "answerer.py").exists():
                warnings.warn(
                    f"answerer module {f} is deprecated / migrate to olf.answerers.Answerer", DeprecationWarning
                )
    ...
    def ask(self, query: str) -> list[BaseAnswer]:
        ...
        for answerer in self[keyword]:
            for answer in answerer.answer(query):
                # In case of *answers* prefix ``answerer:`` is set, see olf.result_types.Result
