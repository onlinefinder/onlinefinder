...
from olf import settings, engines
...
class TestEnginesInit(SearxTestCase):
    ...
    def test_missing_name_field(self):
        ...
        with self.assertLogs('olf.engines', level='ERROR') as cm:  # pylint: disable=invalid-name
            engines.load_engines(engine_list)
            self.assertEqual(len(engines.engines), 0)
            self.assertEqual(cm.output[0], 'ERROR:olf.engines:An engine does not have a "name" field')
    ...
    def test_missing_engine_field(self):
        ...
        with self.assertLogs('olf.engines', level='ERROR') as cm:  # pylint: disable=invalid-name
            ...
            self.assertEqual(len(engines.engines), 0)
            self.assertEqual(
                cm.output[0], 'ERROR:olf.engines:The "engine" field is missing for the engine named "engine2"'
            )
