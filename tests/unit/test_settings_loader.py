...
from olf import settings_loader
...
class TestUserSettings(SearxTestCase):
    ...
    def test_user_settings_not_found(self, path: str):
        with patch.dict(os.environ, {'ONLINEFINDER_SETTINGS_PATH': path}):
            with self.assertRaises(EnvironmentError):
                _s, _m = settings_loader.load_settings()

    def test_user_settings(self):
        with patch.dict(os.environ, {'ONLINEFINDER_SETTINGS_PATH': _settings("user_settings_simple.yml")}):
    ...
    def test_user_settings_remove(self):
        with patch.dict(os.environ, {'ONLINEFINDER_SETTINGS_PATH': _settings("user_settings_remove.yml")}):
            settings, msg = settings_loader.load_settings()
            self.assertTrue(msg.startswith('merge the default settings'))
            self.assertEqual(settings['server']['secret_key'], "user_secret_key")
            self.assertEqual(settings['server']['default_http_headers']['Custom-Header'], "Custom-Value")
    ...
    def test_user_settings_remove2(self):
        with patch.dict(os.environ, {'ONLINEFINDER_SETTINGS_PATH': _settings("user_settings_remove2.yml")}):
            settings, msg = settings_loader.load_settings()
            self.assertTrue(msg.startswith('merge the default settings'))
            self.assertEqual(settings['server']['secret_key'], "user_secret_key")
            self.assertEqual(settings['server']['default_http_headers']['Custom-Header'], "Custom-Value")
    ...
    def test_user_settings_keep_only(self):
        with patch.dict(os.environ, {'ONLINEFINDER_SETTINGS_PATH': _settings("user_settings_keep_only.yml")}):
            settings, msg = settings_loader.load_settings()
            self.assertTrue(msg.startswith('merge the default settings'))
            engine_names = [engine['name'] for engine in settings['engines']]
    ...
    def test_custom_settings(self):
        with patch.dict(os.environ, {'ONLINEFINDER_SETTINGS_PATH': _settings("user_settings.yml")}):
            settings, msg = settings_loader.load_settings()
