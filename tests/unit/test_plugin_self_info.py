...
class PluginIPSelfInfo(SearxTestCase):
    ...
    def setUp(self):
        ...
        self.storage.load_settings({"olf.plugins.self_info.SXNGPlugin": {"active": True}})
        self.storage.init(self.app)
        self.pref = olf.preferences.Preferences(["simple"], ["general"], engines, self.storage)
        self.pref.parse_dict({"locale": "en"})

        cfg = olf.limiter.get_cfg()
        olf.botdetection.init(cfg, None)
    ...
    def test_REMOTE_ADDR_is_invalid(self):
        ...
        with self.assertLogs("olf.botdetection", level="ERROR") as ctx:
            result = self.client.post("/search", data={"q": "ip"}, headers=headers, environ_overrides=environ_overrides)
            self.assertIn(answer, str(result.data))
        self.assertIn(
            "ERROR:olf.botdetection:REMOTE_ADDR: '1.2.3.4.5' does not appear to be an IPv4 or IPv6 address / discard REMOTE_ADDR from WSGI environment",
            ctx.output,
        )
    ...
    def test_X_Real_IP_is_invalid(self):
        ...
        with self.assertLogs("olf.botdetection", level="ERROR") as ctx:
            ...
            self.assertIn(answer, str(result.data))
        self.assertIn(
            "ERROR:olf.botdetection:X-Real-IP: '1.2.3.4.5' does not appear to be an IPv4 or IPv6 address / discard HTTP_X_REAL_IP from WSGI environment",
            ctx.output,
        )
    ...
    def test_X_Forwarded_For_is_invalid(self):
        ...
        with self.assertLogs("olf.botdetection", level="ERROR") as ctx:
            ...
            self.assertIn(answer, str(result.data))
        self.assertIn(
            "ERROR:olf.botdetection:X-Forwarded-For: '1.2.3.4.5' does not appear to be an IPv4 or IPv6 address / discard HTTP_X_FORWARDED_FOR from WSGI environment",
            ctx.output,
        )
