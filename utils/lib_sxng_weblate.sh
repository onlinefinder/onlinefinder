...
weblate.help() {
    cat <<EOF
weblate.:
  push.translations: push translation changes from OnlineFinder to Weblate's counterpart
...
weblate.translations.worktree() {
    ...
    # 'translations' from Weblate's counterpart (weblate) of the OnlineFinder
    # (origin).
    #
    #     remote weblate https://translate.codeberg.org/git/onlinefinder/onlinefinder/

    (
        set -e
        if ! git remote get-url weblate 2>/dev/null; then
            git remote add weblate https://translate.codeberg.org/git/onlinefinder/onlinefinder/
        fi
...
weblate.to.translations() {
    ...
    # Update 'translations' branch of OnlineFinder (origin) with last additions from
    # Weblate.
...
weblate.translations.commit() {
    ...
    (
        ...
        cp -rv --preserve=mode,timestamps "${TRANSLATIONS_WORKTREE}/olf/translations" "olf"
...
weblate.push.translations() {
    ...
    # Push *translation changes* from OnlineFinder (origin) to Weblate's counterpart
    # (weblate).

    # In branch master of OnlineFinder (origin) check for meaningful changes in
    ...
    # 3. Update 'translations' branch of OnlineFinder (origin) with last additions
    #    from Weblate.
    ...
    local messages_pot diff_messages_pot last_commit_hash last_commit_detail \
        exitcode
    messages_pot="${TRANSLATIONS_WORKTREE}/olf/translations/messages.pot"
    (
        set -e
        pyenv.activate
        # get translations branch in git worktree (TRANSLATIONS_WORKTREE)
        ...
        pybabel extract -F babel.cfg --project="OnlineFinder" --version="-" \
            -o "${messages_pot}" \
            "searx/"
    ...
    (
        ...
        pybabel update -N \
            -i "${messages_pot}" \
            -d "${TRANSLATIONS_WORKTREE}/olf/translations"
