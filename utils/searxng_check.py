...
os.environ['ONLINEFINDER_DEBUG'] = '1'

# from here on implement the checks of the installation

import olf

OLD_SETTING = '/etc/olf/settings.yml'

if os.path.isfile(OLD_SETTING):
    msg = (
        '%s is no longer valid, move setting to %s' % (
            OLD_SETTING,
            os.environ.get('ONLINEFINDER_SETTINGS_PATH', '/etc/onlinefinder/settings.yml')
        ))
...
from olf import valkeydb, get_setting
