...
test.:
  ...
  pylint    : lint ./searx, ./onlinefinder_extra and ./tests
...
test.pylint() {
    (
        ...
        build_msg TEST "[pylint] ./olf/engines"
        ...
        build_msg TEST "[pylint] ./searx ./onlinefinder_extra ./tests"
        # shellcheck disable=SC2086
        pylint ${PYLINT_OPTIONS} ${PYLINT_VERBOSE} \
            --ignore-paths=searx/engines \
            searx searx/onlinefinder.msg \
            onlinefinder_extra onlinefinder_extra/docs_prebuild \
            tests
    )
    dump_return $?
}
...
test.themes() {
    build_msg TEST 'OnlineFinder themes'
    themes.test
    dump_return $?
}
