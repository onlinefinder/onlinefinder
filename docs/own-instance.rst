...
.. sidebar:: Is running my own instance worth it?
  ...
  \.\.\.is a common question among OnlineFinder users.  Before answering this
  question, see what options a OnlineFinder user has.
...
instances can be used by a select group of people, such as a OnlineFinder instance for a
...
To gain more insight on how these instances work, let's dive into how OnlineFinder
protects its users.

.. _OnlineFinder protect privacy:

How does OnlineFinder protect privacy?
=================================

OnlineFinder protects the privacy of its users in multiple ways, regardless of the type
...
both cases.  The IP address used will be the IP of the instance, but OnlineFinder can also be
configured to use proxy or Tor.

OnlineFinder does not serve ads or tracking content, unlike most search services.  Therefore,
...
need to use cookies to tailor OnlineFinder to their needs and preferences will not
...
features of OnlineFinder are available to users no matter what kind of instance they
use.

For those on the go, or just wanting to try OnlineFinder for the first time, public
