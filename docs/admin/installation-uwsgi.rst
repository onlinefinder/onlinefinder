...
1. `systemd.unit`_ template file as described here `One service per app in systemd`_:
  ...
  ``onlinefinder.ini`` as example::

    systemd template unit: /usr/lib/systemd/system/uwsgi@.service
            contains: [Service]
                      ExecStart=/usr/bin/uwsgi --ini /etc/uwsgi/%I.ini

    OnlineFinder application:   /etc/uwsgi/onlinefinder.ini
            links to: /etc/uwsgi/apps-available/onlinefinder.ini

  The OnlineFinder app (template ``/etc/uwsgi/%I.ini``) can be maintained as known
  from common systemd units:

  .. code:: sh

     $ systemctl enable  uwsgi@onlinefinder
     $ systemctl start   uwsgi@onlinefinder
     $ systemctl restart uwsgi@onlinefinder
     $ systemctl stop    uwsgi@onlinefinder
...
2. The `uWSGI Emperor`_ which fits for maintaining a large range of uwsgi
  ...
  reload or stopped.  Take Fedora and a ``onlinefinder.ini`` as example::

    to install & start OnlineFinder instance create --> /etc/uwsgi.d/onlinefinder.ini
    to reload the instance edit timestamp      --> touch /etc/uwsgi.d/onlinefinder.ini
    to stop instance remove ini                --> rm /etc/uwsgi.d/onlinefinder.ini
...
The `uwsgi ini file`_ is enabled by a symbolic link::
  ...
  ln -s /etc/uwsgi/apps-available/onlinefinder.ini /etc/uwsgi/apps-enabled/
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      .. kernel-include:: $DOCS_BUILD/includes/onlinefinder.rst
         :start-after: START onlinefinder uwsgi-description ubuntu-20.04
         :end-before: END onlinefinder uwsgi-description ubuntu-20.04
   ...
   .. group-tab:: Arch Linux
      ...
      .. kernel-include:: $DOCS_BUILD/includes/onlinefinder.rst
         :start-after: START onlinefinder uwsgi-description arch
         :end-before: END onlinefinder uwsgi-description arch
   ...
   .. group-tab::  Fedora / RHEL
      ...
      .. kernel-include:: $DOCS_BUILD/includes/onlinefinder.rst
         :start-after: START onlinefinder uwsgi-description fedora
         :end-before: END onlinefinder uwsgi-description fedora
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      .. kernel-include:: $DOCS_BUILD/includes/onlinefinder.rst
         :start-after: START onlinefinder uwsgi-appini ubuntu-20.04
         :end-before: END onlinefinder uwsgi-appini ubuntu-20.04
   ...
   .. group-tab:: Arch Linux
      ...
      .. kernel-include:: $DOCS_BUILD/includes/onlinefinder.rst
         :start-after: START onlinefinder uwsgi-appini arch
         :end-before: END onlinefinder uwsgi-appini arch
   ...
   .. group-tab::  Fedora / RHEL
      ...
      .. kernel-include:: $DOCS_BUILD/includes/onlinefinder.rst
         :start-after: START onlinefinder uwsgi-appini fedora
         :end-before: END onlinefinder uwsgi-appini fedora
...
communication and you want to connect to it from the OnlineFinder uWSGI, you will see a
*Permission denied* in the log of your instance::

  ERROR:searx.valkeydb: [onlinefinder (993)] can't connect valkey DB ...
  ERROR:searx.valkeydb:   Error 13 connecting to unix socket: /usr/local/onlinefinder-valkey/run/valkey.sock. Permission denied.
  ERROR:searx.plugins.limiter: init limiter DB failed!!!

Even if your *onlinefinder* user of the uWSGI process is added to additional groups
to give access to the socket from the valkey DB::

  $ groups onlinefinder
  onlinefinder : onlinefinder onlinefinder-valkey

To see the effective groups of the uwsgi process, you have to look at the status
of the process, by example::

  $ ps -aef | grep '/usr/sbin/uwsgi --ini onlinefinder.ini'
  onlinefinder       93      92  0 12:43 ?        00:00:00 /usr/sbin/uwsgi --ini onlinefinder.ini
  onlinefinder      186      93  0 12:44 ?        00:00:01 /usr/sbin/uwsgi --ini onlinefinder.ini

Here you can see that the additional "Groups" of PID 186 are unset (missing gid
of ``onlinefinder-valkey``)::
