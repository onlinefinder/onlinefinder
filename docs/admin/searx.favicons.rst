...
Activating the favicons in OnlineFinder is very easy, but this **generates a
...
To activate favicons in OnlineFinder's result list, set a default
...
By default and without any extensions, OnlineFinder serves these resolvers:
...
TOML_ configuration is created in the file ``/etc/onlinefinder/favicons.toml``.
...
.. code:: toml
   ...
   db_url = "/var/cache/onlinefinder/faviconcache.db"  # default: "/tmp/faviconcache.db"
...
:py:obj:`cache.db_url <.FaviconCacheConfig.db_url>`:
  ...
  cache of applications, so a suitable storage location of OnlineFinder's caches is
  folder ``/var/cache/onlinefinder``.

  In a standard installation (compare :ref:`create onlinefinder user`), the folder
  must be created and the user under which the OnlineFinder process is running must
  be given write permission to this folder.

  .. code:: bash

     $ sudo mkdir /var/cache/onlinefinder
     $ sudo chown root:onlinefinder /var/cache/onlinefinder/
     $ sudo chmod g+w /var/cache/onlinefinder/
...
OnlineFinder hosters can change other parameters of the cache as required:
...
already set sensibly with settings from the :ref:`settings.yml <onlinefinder
...
:py:obj:`max_age <.FaviconProxyConfig.max_age>`:
  ...
  As a rule, in the favicons infrastructure of OnlineFinder's this setting only
...
infrastructure of OnlineFinder generates this ``resolver_map`` automatically
depending on the ``settings.yml``.  OnlineFinder would automatically generate the
...
In the following we list the resolvers available in the core of OnlineFinder, but via
