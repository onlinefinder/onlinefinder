...
.. _DockerHub mirror: https://hub.docker.com/r/onlinefinder/onlinefinder
.. _GHCR mirror: https://ghcr.io/onlinefinder/onlinefinder
.. _Docker compose: https://github.com/onlinefinder/onlinefinder-docker
...
.. important::
   ...
   maintaining your OnlineFinder instance.  This guide assumes familiarity with
...
.. code:: sh
   ...
   $ docker pull docker.io/onlinefinder/onlinefinder:latest

\.\. or if you want to lock in to a specific version:

.. code:: sh

   $ docker pull docker.io/onlinefinder/onlinefinder:2025.8.1-3d96414
...
.. code:: sh
   ...
   $ mkdir -p ./onlinefinder/config/ ./onlinefinder/data/
   $ cd ./onlinefinder/

   # Run the container
   $ docker run --name onlinefinder -d \
       -p 8888:8080 \
       -v "./config/:/etc/onlinefinder/" \
       -v "./data/:/var/cache/onlinefinder/" \
       docker.io/onlinefinder/onlinefinder:latest

This will start OnlineFinder in the background, accessible at http://localhost:8888
...
.. code:: sh
   ...
   1af574997e63  ...    ...  3 minutes ago  0.0.0.0:8888->8080/tcp  onlinefinder

Access the container shell (troubleshooting):

.. code:: sh

   $ docker container exec -it --user root onlinefinder /bin/sh -l
   1af574997e63:/usr/local/onlinefinder#

Stop and remove the container:

.. code:: sh

   $ docker container stop onlinefinder
   $ docker container rm onlinefinder
...
- ``/etc/onlinefinder``: Configuration files (settings.yml, etc.)
- ``/var/cache/onlinefinder``: Persistent data (faviconcache.db, etc.)
...
- ``$ONLINEFINDER_*``: Controls the OnlineFinder configuration options, look out for
  environment ``$ONLINEFINDER_*`` in :ref:`settings server` and :ref:`settings
  general`.
...
- ``$FORCE_OWNERSHIP``: Ensures mounted volumes/files are owned by the
  ``onlinefinder:onlinefinder`` user (default: ``true``)

Container internal paths (don't modify unless you know what you're doing):

- ``$CONFIG_PATH``: Path to the OnlineFinder configuration directory (default: ``/etc/onlinefinder``)
- ``$ONLINEFINDER_SETTINGS_PATH``: Path to the OnlineFinder settings file (default: ``$CONFIG_PATH/settings.yml``)
- ``$DATA_PATH``: Path to the OnlineFinder data directory (default: ``/var/cache/onlinefinder``)
...
To build your own OnlineFinder container image from source (please note, custom
container images are not officially supported):

.. code:: sh

   $ git clone https://github.com/onlinefinder/onlinefinder.git
   $ cd ./onlinefinder/
   ...
   localhost/onlinefinder/onlinefinder  2025.8.1-3d96414    ...       About a minute ago  183 MB
   localhost/onlinefinder/onlinefinder  latest              ...       About a minute ago  183 MB
   localhost/onlinefinder/onlinefinder  builder             ...       About a minute ago  524 MB
   ghcr.io/onlinefinder/base       onlinefinder-builder     ...       2 days ago          378 MB
   ghcr.io/onlinefinder/base       onlinefinder             ...       2 days ago          42.2 MB
