...
``request_timeout`` :
  ...
  will slow OnlineFinder reactivity (the result page may take the time specified in the
  timeout to load).  Can be override by ``timeout`` in the :ref:`settings engines`.

``useragent_suffix`` :
  Suffix to the user-agent OnlineFinder uses to send requests to others engines.  If an
...
``retries`` :
  Number of retry in case of an HTTP error.  On each retry, OnlineFinder uses an
...
``verify``: : ``$SSL_CERT_FILE``, ``$SSL_CERT_DIR``
  ...
  In addition to ``verify``, OnlineFinder supports the ``$SSL_CERT_FILE`` (for a file) and
