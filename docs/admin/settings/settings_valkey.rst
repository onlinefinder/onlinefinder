...
find a description to test your valkey connection in OnlineFinder.

``url`` : ``$ONLINEFINDER_VALKEY_URL``
  ...
  When using sockets, don't forget to check the access rights on the socket::
    ...
    ls -la /usr/local/onlinefinder-valkey/run/valkey.sock
    srwxrwx--- 1 onlinefinder-valkey onlinefinder-valkey ... /usr/local/onlinefinder-valkey/run/valkey.sock

  In this example read/write access is given to the *onlinefinder-valkey* group.  To
  get access rights to valkey instance (the socket), your OnlineFinder (or even your
  developer) account needs to be added to the *onlinefinder-valkey* group.
...
.. code:: sh
   ...
   $ ./utils/onlinefinder.sh install valkey
   # restart your OnlineFinder instance
