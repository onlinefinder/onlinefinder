...
.. attention::
   ...
   The ``enabled_plugins:`` section in OnlineFinder's settings no longer exists.
...
In OnlineFinder, plugins can be registered in the :py:obj:`PluginStore
...
``/etc/onlinefinder/settings.yml`` must be overwritten.
...
OnlineFinder supports *external plugins* / there is no need to install one, OnlineFinder
runs out of the box.
