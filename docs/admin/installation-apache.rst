...
This section explains how to set up a OnlineFinder instance using the HTTP server Apache_.
If you did use the :ref:`installation scripts` and do not have any special preferences
you can install the :ref:`OnlineFinder site <apache onlinefinder site>` using
:ref:`onlinefinder.sh <onlinefinder.sh overview>`:

.. code:: bash

   $ sudo -H ./utils/onlinefinder.sh install apache
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      In :ref:`Debian's Apache layout` you create a ``onlinefinder.conf`` with the
      ``<Location /onlinefinder >`` directive and save this file in the *sites
      available* folder at ``/etc/apache2/sites-available``.  To enable the
      ``onlinefinder.conf`` use :man:`a2ensite`:

      .. code:: bash

         sudo -H a2ensite onlinefinder.conf
   ...
   .. group-tab:: Arch Linux
      ...
      .. code:: bash
         ...
         sudo -H ln -s /etc/httpd/sites-available/onlinefinder.conf \
                       /etc/httpd/sites-enabled/onlinefinder.conf
...
.. _apache onlinefinder site:

Apache's OnlineFinder site
=====================
...
To proxy the incoming requests to the OnlineFinder instance Apache needs the
...
Depending on what your OnlineFinder installation is listening on, you need a http
...
You can install and activate your own ``onlinefinder.conf`` like shown in
:ref:`apache sites`.
...
.. code:: apache
   ...
   SetEnvIf Request_URI "/onlinefinder" dontlog
   # CustomLog /dev/null combined env=dontlog

The ``CustomLog`` directive disables logs for the entire (virtual) server, use it
when the URL of the service does not have a path component (``/onlinefinder``), so when
OnlineFinder is located at root (``/``).
