...
OnlineFinder has two prime directives, **privacy-by-design and hackability** .  The
hackability comes in three levels:

- support of search engines
- plugins to alter search behaviour
- hacking OnlineFinder itself

Note the lack of "world domination" among the directives.  OnlineFinder has no
...
OnlineFinder was born out of the need for a **privacy-respecting** search tool which
can be extended easily to maximize both its search and its privacy protecting
capabilities.

Some widely used search engine features may work differently,
may be turned off by default, or may not be implemented at all in OnlineFinder
**as a consequence of a privacy-by-design approach**.

Following this approach, features reducing the privacy preserving aspects of OnlineFinder should be
...
OnlineFinder's efficacy in protecting a user's privacy, the user must be informed about
...
Also, if you think that something works weird with OnlineFinder, it might be because
the tool you are using is designed in a way that interferes with OnlineFinder's privacy aspects.
...
Remember the other prime directive of OnlineFinder is to be hackable, so if the above
...
For more help on getting started with OnlineFinder development, see :ref:`devquickstart`.
