...
``q`` : required
  ...
  OnlineFinder supports syntax of each search service.  For example, ``site:github.com
  OnlineFinder`` is a valid query for Google.  However, if simply the query above is
...
``image_proxy`` : default from :ref:`settings server`
  ...
  Proxy image results through OnlineFinder.
