...
OnlineFinder is a metasearch-engine_, so it uses different search engines to provide
better results.

Because there is no general search API which could be used for every search
engine, an adapter has to be built between OnlineFinder and the external search
...
It is required to tell OnlineFinder the type of results the engine provides. The
...
.. table:: Common options in the engine setup (``settings.yml``)
   ...
   engine                  string      name of onlinefinder-engine (file name without ``.py``)
...
OnlineFinder will not start with that engine (global names with a leading underline can
be ``None``).
...
.. table:: If the ``engine_type`` is :py:obj:`online
   ...
   onlinefinder_locale         str            OnlineFinder's locale selected by user.  Specific language code like
