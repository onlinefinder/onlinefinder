...
To extend the functionality of OnlineFinder, offline engines are going to be
...
If an offline engine depends on an external tool, OnlineFinder does not install it by
...
Environment* of your OnlineFinder instance you need to switch into the environment
(:ref:`onlinefinder-src`) first, for this you can use :ref:`onlinefinder.sh`::

  $ sudo utils/onlinefinder.sh instance cmd bash
  (onlinefinder-pyenv)$ pip install ...
...
query contains a valid token, then OnlineFinder performs the requested private
