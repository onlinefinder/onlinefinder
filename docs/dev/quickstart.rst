...
OnlineFinder loves developers; Developers do not need to worry about tool chains, the
usual developer tasks can be comfortably executed via :ref:`make <makefile>`.

Don't hesitate, just clone OnlineFinder's sources and start hacking right now ..
...
.. _mise.toml:
    https://github.com/onlinefinder/onlinefinder/blob/master/mise.toml
.. _mise en place:
...
``make run`` (2.) which starts a :ref:`onlinefinder granian` server you can jump
into the developer environment and start a python based HTTP server by::

    $ ./manage dev.env
    ...
    (dev.env)$ ONLINEFINDER_DEBUG=1 python -m searx.webapp
