...
project = 'OnlineFinder'
copyright = 'OnlineFinder team'
author = 'OnlineFinder team'
release, version = VERSION_STRING, VERSION_STRING
ONLINEFINDER_URL = get_setting('server.base_url') or 'https://example.org/onlinefinder'
...
SOURCEDIR = Path(__file__).parent.parent / "olf"
...
# import olf.webapp is needed to init the engines & plugins, to init a
# (empty) secret_key is needed.
olf.settings['server']['secret_key'] = ''
import olf.webapp

olf.engines.load_engines(olf.settings['engines'])

jinja_contexts = {
    'olf': {
        'engines': olf.engines.engines,
        'plugins': olf.plugins.STORAGE,
        'version': {
            'node': os.getenv('NODE_MINIMUM_VERSION')
        },
        'enabled_engine_count': sum(not x.disabled for x in olf.engines.engines.values()),
        'categories': olf.engines.categories,
        'categories_as_tabs': {c: olf.engines.categories[c] for c in olf.settings['categories_as_tabs']},
    },
}
jinja_filters = {
    'group_engines_in_tab': olf.webutils.group_engines_in_tab,
}
...
def setup(app):
    ...
    def source_read(app, docname, source):
        if docname == ENGINES_DOCNAME:
            jinja_contexts['olf']['documented_modules'] = app.env.domains['py'].modules
...
# upstream links
extlinks['wiki'] = ('https://github.com/onlinefinder/onlinefinder/wiki/%s', ' %s')
extlinks['pull'] = ('https://github.com/onlinefinder/onlinefinder/pull/%s', 'PR %s')
extlinks['pull-olf'] = ('https://github.com/olf/olf/pull/%s', 'PR %s')
...
issues_github_path = "onlinefinder/onlinefinder"

# HTML -----------------------------------------------------------------

# https://onlinefinder.github.io/onlinefinder --> '/onlinefinder/'
# https://docs.onlinefinder.org --> '/'
notfound_urls_prefix = '/'

sys.path.append(os.path.abspath('_themes'))
sys.path.insert(0, os.path.abspath("../"))
html_theme_path = ['_themes']
html_theme = "onlinefinder"
...
html_logo = "../client/simple/src/brand/onlinefinder-wordmark.svg"
html_title = "OnlineFinder Documentation ({})".format(VERSION_STRING)
html_show_sourcelink = True

# LaTeX ----------------------------------------------------------------

latex_documents = [
    (master_doc, "onlinefinder-{}.tex".format(VERSION_STRING), html_title, author, "manual")
]
