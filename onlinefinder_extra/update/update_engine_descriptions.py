...
:origin:`olf/engines/wikidata.py` engine.

Output file: :origin:`olf/data/engine_descriptions.json`.
...
from olf.engines import wikidata, set_loggers
from olf.utils import extract_text, onlinefinder_useragent
from olf.locales import LOCALE_NAMES, locales_initialize, match_locale
from olf import searx_dir
from olf.utils import gen_useragent, detect_language
import olf.search
import olf.network
from olf.data import data_dir
...
def get_wikipedia_summary(wikipedia_url, onlinefinder_locale):
    # get the REST API URL from the HTML URL

    # Headers
    headers = {'User-Agent': onlinefinder_useragent()}

    if onlinefinder_locale in WIKIPEDIA_LANGUAGE_VARIANTS:
        headers['Accept-Language'] = WIKIPEDIA_LANGUAGE_VARIANTS.get(onlinefinder_locale)
    ...
    try:
        response = olf.network.get(wikipedia_rest_url, headers=headers, timeout=10)
...
def get_website_description(url, lang1, lang2=None):
    ...
    try:
        response = olf.network.get(url, headers=headers, timeout=10)
        response.raise_for_status()
...
def initialize():
    ...
    wikipedia_engine = olf.engines.engines['wikipedia']
    ...
    for engine_name, engine in olf.engines.engines.items():
...
def fetch_wikidata_descriptions():
    ...
    olf.network.set_timeout_for_thread(60)
    ...
    if result is not None:
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for onlinefinder_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[onlinefinder_locale] != wikidata_lang:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                        f"/ len(wikidata_desc): {len(desc)}",
                    )
                    update_description(engine_name, onlinefinder_locale, desc, 'wikidata')
        ...
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for onlinefinder_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[onlinefinder_locale] != wikidata_lang:
                        continue
                    desc = get_wikipedia_summary(wikipedia_url, onlinefinder_locale)
                    if not desc:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                    ...
                    update_description(engine_name, onlinefinder_locale, desc, 'wikipedia')
