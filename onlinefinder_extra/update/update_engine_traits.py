...
"""Update :py:obj:`olf.enginelib.traits.EngineTraitsMap` and :origin:`olf/languages.py`

:py:obj:`olf.enginelib.traits.EngineTraitsMap.ENGINE_TRAITS_FILE`:
  Persistence of engines traits, fetched from the engines.

:origin:`olf/languages.py`
  Is generated  from intersecting each engine's supported traits.

The script :origin:`onlinefinder_extra/update/update_engine_traits.py` is called in
...
from olf import settings, searx_dir
...
# SPDX-License-Identifier: AGPL-3.0-or-later
'''List of OnlineFinder's locale codes used for the search language/region.
...
0. OnlineFinder's internal locale tag (a language or region tag)
...
4. Unicode flag (emoji) that fits to OnlineFinder's internal region tag. Languages
